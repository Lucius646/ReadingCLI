...
v0.1 已经可以作为一个基础终端小说阅读器使用：

- 使用 `cargo run -- open <txt-path>` 打开本地小说。
- 进入全屏 TUI 阅读界面。
- 根据终端宽高动态分页，窗口大小变化后自动重建分页索引。
- 使用 `n` 下一页、`p` 上一页、`q` 退出。
- 保存当前阅读 offset，下次打开同一本书时恢复进度。
- 支持 UTF-8 文本，并在 UTF-8 解码失败时 fallback 到 GBK。
- 状态栏显示当前页、总页数、文件 offset 和阅读百分比。
